...
void c_Sim::execute()
{
    ...
    cout << endl
         << "Beginning main loop with num_cells=" << num_cells << " and timestep=" << dt << " cflfactor=" << cflfactor << " and num_species = " << num_species << endl;
    if (num_species == 0)
    ...
    for (globalTime = 0; (globalTime < t_max) && (steps < maxsteps);)
    {
        ...
        if (start_hydro_time > 0. && globalTime > start_hydro_time)
        { // Comment in if a radiative equilibrium phase is desired before starting hydro
            do_hydrodynamics = 1;
        }
        ...
        if ((photochemistry_level + use_rad_fluxes) > 0)
        {
            ...
            { // Linearized time-dependent general chemistry scheme
                if (steps > 2)
                {
        ...
        else if (use_collisional_heating && num_species > 1)
        ...
        for (int s = 0; s < num_species; s++)
        {
            ...
            {
                cout << endl
                     << ">>> CRASH <<< DUE TO NEGATIVE TEMPERATURES, crash_imin/imax = " << crash_T_imin << "/" << crash_T_imax << " sample T = " << crashed_temperature << " num of crashed cells/total cells = " << crash_T_numcells << "/" << num_cells << "  crashed species name = " << species[crashed_T - 1].speciesname << endl;
            ...
            for (int b = 0; b < num_bands_out; b++)
            {
                for (int i = num_cells - 1; i >= 0; i--)
                {
                    if (Jrad_FLD(i, b) < 0)
                    {
                        ...
                        { // Ignore extremely small flux overshoots
                            Jrad_FLD(i, b) *= -1.;
                        }
                        else
                        {
                ...
                {
                    cout << endl
                         << ">>> CRASH <<< DUE TO NEGATIVE J, crash_imin/imax = " << crash_J_imin << "/" << crash_J_imax << " sample J = " << crashed_meanintensity << " num of crashed cells/total cells = " << crash_J_numcells << "/" << num_cells << "  crashed band number = " << crashed_J - 1 << endl;
                    cout << " @t/dt = " << crashtime << "/" << dt << " stepnum " << steps << endl;
                    cout << "Writing crash dump into last output and exiting program." << endl;
                }
            }
...
void c_Species::execute(std::vector<AOS> &u_in, std::vector<AOS> &dudt)
{
    ...
    if (debug >= 2)
        cout << "Done." << endl
             << " Before compute pressure in species " << speciesname << "... ";

    if (base->debug > 0)
    { // Can put in custom debug info here. We left some example code in just for referencing the code usage.
        ...
        if ((base->steps == 3205600) || (base->steps == 3206000))
        { // Narrow down problematic time

            for (int j = 19; j <= 22; j++)
            { // Narrow down problematic cells, in this case rad fluxes do something unexpected
    ...
    for (int j = 1; j <= num_cells; j++)
    {
        ...
        source_pressure[j] = AOS(0, -(base->source_pressure_prefactor_left[j] * prim_l[j].pres - base->source_pressure_prefactor_right[j] * prim_r[j].pres), 0);
    }
    ...
    for (int j = 1; j <= num_cells; j++)
    {
        ...
        { // Or put in your own conditions
            ...
            cout << " " << endl;
            cout << "     sP = "
                 << "/" << source_pressure[j].u1 << "/" << source_pressure[j].u2 << "/" << source_pressure[j].u3 << endl;
