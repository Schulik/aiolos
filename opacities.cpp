...
void c_Species::update_opacities() {
    ...
    int htwo_idx = -1; //base->get_species_index("H2",0); This was for a hack, reverting
    int c_idx    = base->C_idx;
    int o_idx    = base->O_idx;
    int h2o_idx  = base->h2o_idx;	
    ...
    if (base->opacity_model == 'U') {
        ...
        if(is_dust_like == 0) {
            ...
            for(int j=0; j< num_cells+2; j++) {
                ...
                for(int b=0; b<num_bands_out; b++) {
                    opacity(j,b)         = base->const_opacity_rosseland_factor * 1.0; //base->const_opacity_rosseland_factor * opacity_avg_rosseland(b);// * (1. + pressure_broadening_factor * pow(prim[j].pres/1e5, pressure_broadening_exponent)); 
