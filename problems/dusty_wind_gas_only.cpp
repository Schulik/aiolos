...
void c_Sim::user_loop_function() {
    ...
    int j = num_ghosts;
    SurfaceCondensation planet_surf(
        silicate, species[0].mass_amu, species[0].cv, species[1].cv,
        RHO_DUST * THICKNESS, surf[j - 1] / vol[j]);
    if (use_rad_fluxes == 1) {
        ...
        // Thermal radiation reaching the planet (F_down = \pi J in diffusion
        // limit)
    ...
    if (species[1].prim[j].speed < 0) {
        ...
        double E_tot = prim.density * (species[1].cv * prim.temperature +
    ...
    planet_surf.set_state(T_core, species[0].prim[j].temperature,
                          species[0].prim[j].pres, rad, L_rain, dt);
    ...
    */
    ...
    for (int s = 0; s < num_species; s++) {
        ...
        species[s].eos->compute_conserved(&species[s].prim[j],
                                          &species[s].u[j], 1);
    }
};
