        ...
        for (int j=0; j < num_cells+1; j++) {
            ...
            if (j > 0 && j < num_cells + 1) {
               ...
               /* if(j > num_cells-3) {
                ...
                double R       = 2. * tau_inv *  dJ ; // Put in 1.0 as prefactor to get correct rad shock
                double D       = no_rad_trans * tau_inv * no_rad_trans * surf[j] * flux_limiter(R);
    ...
    if(radiation_matter_equilibrium_test <= 2) { //radtests 3 and 4 are delta-radiation peaks without energy-matter coupling
        ...
        for (int j=0; j < num_cells+1; j++) {
            ...
            for (int s=0; s < num_species; s++) {
                ...
                exchange_d_sum += rhos * kappa * (1 - 4.*sigma_rad*Ts3/pi * eta2[idx_s]/denoms[idx_s]);
                exchange_r_sum += rhos * kappa * sigma_rad * Ts3/pi * (4 * eta1[idx_s]/denoms[idx_s] - 3 * Ts );
            }
