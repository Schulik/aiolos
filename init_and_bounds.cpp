...
c_Sim::c_Sim(string filename_solo, string speciesfile_solo, string workingdir, string tintent, std::vector<int> debug_data, int restartnumber, double restarttime_cmdline) {
	...
	star_mass        *= msolar;
        ...
        if(type_of_grid == 0) {
            ...
            if(type_of_grid==2) {
                    ...
                    grid2_transition_cells = read_parameter_from_file<int>(filename,"GRID2_TRANS_CELLS", debug, 20).value; // For bi-logarithmic grid, number cells per decade in resolution in second region at large r (first region sits at small r)
	...
	avg_temperature_t1  = read_parameter_from_file<double>(filename,"AVG_TEMPERATURE_T1", debug, 1e99).value; //All T_s remain T_s after t1. T_avg is ignored after t1
        ...
        use_chem_reaction_floor           = read_parameter_from_file<int>(filename,"USE_CHEM_REACTIONFLOOR", debug, 0).value; //Use a floor on the reaction rates to prevent tiny rates from blowing up the solver?
        ...
        if(type_of_grid==1) {
            ...
            for(int i=1; i<= num_cells; i++) {
                ...
                if(i <= grid2_transition_i) {
                    ...
                    templogx = dlogx;
                } else if (i >= (grid2_transition_i + grid2_transition_cells)) {
                    templogx = dlogx2;
                } else {
                    templogx = dlogx + double(i-grid2_transition_i)/((double)grid2_transition_cells) * (dlogx2 - dlogx);
                }
    ...
    radial_optical_depth_twotemp_he = Eigen::MatrixXd::Zero(num_cells+2, num_bands_in);
    ...
    implicit_tridiag = BlockTriDiagSolver<Eigen::Dynamic>(num_cells+2, 3) ;
...
void c_Species::apply_boundary_left(std::vector<AOS>& u) {
    ...
    switch(boundary_left) {
        ...
        case BoundaryType::periodic:  //enum type 4
            ...
            break;
        case BoundaryType::giantplanet: //enum type 5
            for (int i=0; i < num_ghosts; i++) {
                int igh =  num_ghosts-1 -i;
                int iact = num_ghosts   +i;
                u[igh]     = u[iact]; 
 ...
 * @param[in] opafile   opacityfile to compute ionisation rates
 */
c_photochem_reaction::c_photochem_reaction(int num_species, int num_bands, int band, std::vector<int> e_indices, std::vector<int> p_indices, std::vector<double> e_stoch, std::vector<double> p_stoch, double branching, double threshold, string opafile)
{
        ...
        this->opacity_twotemp = Eigen::VectorXd::Zero(num_bands);
...
//
//
void c_Sim::init_highenergy_opacities() {
