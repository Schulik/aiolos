...
void c_Sim::transport_radiation() {
        ...
        for(int s = 0; s < num_species; s++)
            for(int j = 0; j< num_cells+2; j++) {
                T_FLD(j,s)          = species[s].prim[j].temperature;
            }
        
        correct_totalenergy(1.0*dt, Etot_corrected);
        ...
        while( ( iters < rad_solver_max_iter) && (epsilon_rad > epsilon_rad_min) ) {
            ...
            //update_temperatures(0.5*dt); 
            
            //update_fluxes_FLD(1.0*dt, 0., Jrad_FLD2);
            ...
            //update_fluxes_FLD(1.0*dt,   Jrad_FLD, Jrad_FLD3, T_FLD2); //J_in, J_out, T_in
            //update_temperatures(1.0*dt, Jrad_FLD3, T_FLD, T_FLD3); // J_in,  T_in, T_out
            ...
            //Jrad_FLD = 1.0 * (Jrad_FLD2 + Jrad_FLD3); //Jrad_FLD3; 
            //Jrad_FLD = Jrad_init;
            //Jrad_FLD = Jrad_FLD3; 
            
            //update_fluxes_FLD(0.5*dt, 1., Jrad_FLD, Jrad_FLD3, T_FLD2); //Eigen::MatrixXd &J_in, Eigen::MatrixXd &J_target, Eigen::MatrixXd &T_in
            //update_temperatures(0.5*dt, Jrad_FLD3, T_FLD2, T_FLD3); // Eigen::MatrixXd &J_in, Eigen::MatrixXd &T_in, Eigen::MatrixXd &T_out
            
            
            if(steps == 1 || steps== 0) {
                cout<<"   in transport_radiation, after update temperatures."<<endl;
                cout<<"Jrad_FLD = "<<endl<<Jrad_FLD<<endl;
            }
             if(debug >= 1)
                cout<<"   in transport_radiation, finished everything."<<endl;
            //update_fluxes_FLD(0.5*dt);
            
            iters++;
        }
        
        //
        // After we're done, save T and J, except we do the radiation-matter equilibrium test, in which case J is kept constant
        //
        if( radiation_matter_equilibrium_test == 1)
            Jrad_FLD = Jrad_init;
        else
            //Jrad_FLD = 0.5*(Jrad_FLD3 + Jrad_FLD2 );
            Jrad_FLD = Jrad_FLD2;
        
        for(int j =0; j< num_cells+2; j++){
            Jrad_FLD(j,0) += 0.* c_light/4./pi*Etot_corrected(j); 
        }
        
        for(int s = 0; s < num_species; s++)
             for(int j = 0; j < num_cells+2; j++) {
                 //species[s].prim[j].temperature = 0.5*(T_FLD3(j,s)+T_FLD2(j,s));
                 species[s].prim[j].temperature = T_FLD2(j,s);
             }
...
void c_Species::update_opacities() {
    ...
    for(int j=0; j< num_cells+2; j++) {
        for(int b=0; b<num_bands; b++) {
            opacity(j,b)        = const_opacity; // /(base->x_i12[j] * base->x_i12[j] )/(base->l_i[b]+base->l_i[b+1]); //TODO: Replace with some_complex_tabulated_opacity_function();
            opacity_planck(j,b) = const_opacity; // /(base->x_i12[j] * base->x_i12[j] )/(base->l_i[b]+base->l_i[b+1]); //TODO: Replace with some_complex_tabulated_opacity_function();
        }
    }
}

void c_Sim::update_fluxes_FLD(double timestep, Eigen::MatrixXd &J_in, Eigen::MatrixXd &J_target, Eigen::MatrixXd &T_in) {
    ...
    for(int b=0; b<num_bands; b++) {
        ...
        for (int j=0; j < num_cells+1; j++) {
            ...
            double D       = 0.* surf[j] * flux_limiter(R) * tau_inv;
            
            //cout<<" in FLD, band "<<b<<" tau_inv = "<<tau_inv<<" opa1 = "<<total_opacity(j,b)<<" opa2 = "<<total_opacity(j+1,b)<<" dx = "<<dx<<" x1 = "<<x_i12[j]<<" x2 = "<<x_i12[j+1] <<" x1-x2 = "<<(x_i12[j+1]-x_i12[j])<< " end"<<endl;
            /*
            saveR = R;
            saveL = flux_limiter(R);
            savetau = tau_inv;
            saveJ1  = J_in(j,b);
            saveJ2  = J_in(j+1,b);
            */
            ...
            d[j] += vol[j] / (c_light * timestep) ;
            r[j] += (vol[j] / (c_light * timestep)) * J_in(j, b) ;
        }
        
//         for (int j=0; j < num_cells+1; j++) {
//             if(globalTime > 1.) {
//                         cout<<" timeswitch = "<<timeswitch<<" netflux in cell "<<j<<" = "<<u[j]-2.*d[j]+l[j]<<" r = "<<r[j]<<endl;
//                     
//             }
//         }
        ...
        if(debug >= 1) {
            ...
            double f1 = species[0].prim[2].density * species[0].opacity_planck(2);
            double f2 = f1 * species[0].opacity_planck(2)*sigma_rad*pow(T_in(2,0),4) / pi;
            double result = (J_in(2,0)*invcdt + f2)/(invcdt+f1);
            
            cout<<"    in update flux J_old = "<<J_in(2,0)*invcdt<<" cdtkrho = "<<(invcdt+f1)<<" cdtkrhosigmaT^4 = "<<(f2)<< " result = "<<result;
            /*
            for(int b=0; b<num_bands;b++) {
                cout<<"    band ["<<b<<"] ";
                for(int s=0; s<num_species; s++) {
