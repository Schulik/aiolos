...
AOS c_Species::source_grav_noconserved(int &j) {
    assert(j > 0 && j <= num_cells) ;
...
void c_Species::update_kzz_and_gravpot(int argument) {
    ...
    if(argument > 0 && mi > 1e-3) { //Do the kzz buildup for all other species except the lower atmospheric dominant species (assumed species[0]) and no electrons (mass = 5e-4).
    ...
    if(homopause_boundary_i > 0) {
        ...
        phi_s[homopause_boundary_i] = -std::pow(10.,0.5*qq);
    }
    if(homopause_boundary_i > 0) {
        double qq = std::log10( -phi_s[homopause_boundary_i] ) + std::log10(- phi_s[homopause_boundary_i+2]);
        phi_s[homopause_boundary_i+1] = -std::pow(10.,0.5*qq);
    }
    if(homopause_boundary_i > 1) {
        double qq = std::log10( -phi_s[homopause_boundary_i-2] ) + std::log10(- phi_s[homopause_boundary_i]);
        phi_s[homopause_boundary_i-1] = -std::pow(10.,0.5*qq);
    }*/
...
AOS c_Species::source_diffusion_flux(int j) {
    ...
    double fjm1   = std::log(prim[j].number_density/base->total_numdens[j]);  
    double fj     = std::log(prim[j+1].number_density/base->total_numdens[j+1]);
...
double c_Species::diffusive_timestep(int j) {
    ...
    AOS flux = source_diffusion_flux2(j, true); //Uses get_v=true switch!
...
void c_Sim::execute_separate_diffusion_step() {
    ...
    for(int s=0; s<num_species; s++) {
        for(int j=0; j < num_cells+1; j++) {
            ...
            double mean_temp = return_T_mean(j);
