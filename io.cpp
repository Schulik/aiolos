        ...
        for (int col = 1; col < num_readin_columns+1; col++) {
            ...
            for(int i = 0; i< num_tmp_lambdas; i++) {
                ...
                if(i==0)
                    ...
                    if(std::log10(lmax/lmin) > 1e-20)
                        if( (file_opacity_data(j,col)>1e-20) && (file_opacity_data(j+1,col) > 1e-20) )
                            m  =  std::log10(file_opacity_data(j+1,col) / file_opacity_data(j,col) ) / std::log10(lmax/lmin);
...
void c_Species::print_AOS_component_tofile(int timestepnumber) {
    ...
    string filename ;
    {
        ...
        if(base->restartnumber != 0)
            restartstring = "RE";
        
        stringstream filenamedummy;
        string truncated_name = stringsplit(base->simname,".")[0];
        filenamedummy<<base->workingdir<<"output_"<<truncated_name<<"_"<<speciesname<<"_t"<<timestepnumber<<restartstring<<".dat";
    ...
    if (outfile.is_open())
    {
        ...
        for(int i=1; i<= num_cells; i++) {
            ...
            outfile<<ri<<'\t'<<u[i].u1<<'\t'<<u[i].u2<<'\t'<<u[i].u3<<'\t'<<u[i].u1/mass_amu/amu<<'\t'<<12.*u[i].u2*ri*ri<<'\t'<<12.*u[i].u2*ri*ri/mass_amu/amu<<'\t'<<prim[i].number_density/base->total_numdens[i]<<'\t'<<base->total_press[i]/1e6<<'\t'<<balance3<<'\t'<<prim[i].pres<<'\t'<<u[i].u2/u[i].u1<<'\t'<<prim[i].temperature <<'\t'<<timesteps_cs[i]<<'\t'<<base->cflfactor/timesteps[i]<<'\t'<<prim[i].sound_speed<<'\t'<<timesteps_de[i]<<'\t'<<source_diffusion_flux2(i,true).u1<<'\t'<<dQ_hydro(i)<<'\t'<<-phi_s[i]<<'\t'<<base->enclosed_mass_tmp[i]<<'\t'<<dG(i)+0.*dGdT(i)*prim[i].temperature <<'\t'<<dS(i)<<'\t'<<base->cell_optical_depth(i,0)<<endl;
        } 
...
void c_Sim::print_diagnostic_file(int outputnumber) {
    ...
    //Only print this file once, as it contains all the info 
        string filenameDiagnostic ;
        {
            ...
            filenamedummy<<workingdir<<"diagnostic_"<<truncated_name<<"_t"<<outputnumber<<restartstring<<".dat";
...
Eigen::VectorXd c_Sim::read_and_bin_opacityfile(string filename, int col) {

	Eigen::VectorXd opacity_avg_final = Eigen::VectorXd::Zero(num_bands_in);
        int num_opacity_datas = -1;
        //
        // Start reading opacity data
        //
        
        string opacityinputfile = "inputdata/" + filename;
        ifstream file2( opacityinputfile);
        string line2;
        ...
        Eigen::MatrixXd opacity_data = Eigen::MatrixXd::Zero(num_tmp_lambdas, num_readin_columns + 1);   
	...
	//End averaging
        ...
        for(int b = 0; b < num_bands_in; b++) if(std::isnan(opacity_avg_final(b) )) opacity_avg_final(b) = minimum_opacity;
        for(int b = 0; b < num_bands_in; b++) if((opacity_avg_final(b)< minimum_opacity )) opacity_avg_final(b) = minimum_opacity;
        ...
        //
        //cout<<"        avg opacities solar = "<<endl;
        //for(int b = 0; b < num_bands_out; b++) cout<<"<"<<base->l_i_in[b]<<" - "<<base->l_i_in[b+1]<<" mum > "<<opacity_avg(b)<<endl; //"/";
        //cout<<endl;
        
	return opacity_avg_final;
    }
